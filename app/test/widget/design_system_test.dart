...
void main() {
  ...
  group('LdNextActionCard', () {
    const action = NextAction(
      id: 'protein_debt',
      domain: ActionDomain.nutrition,
      title: 'You are 32 g below your protein target',
      subtitle: '4 h until bed · a 40 g snack closes it',
      actionLabel: 'Log protein',
      deeplink: '/nutrition/log',
      priority: 87,
      evidence: [
        (label: 'Protein today', value: '168 g'),
        (label: 'Protein floor', value: '200 g'),
      ],
    );

    testWidgets('renders the action and both affordances', (tester) async {
      await tester.pumpWidget(
        host(LdNextActionCard(action: action, onAct: () {}, onWhy: () {})),
      );
      expect(find.text('You are 32 g below your protein target'), findsOneWidget);
      expect(find.text('Log protein'), findsOneWidget);
      expect(find.text('Why?'), findsOneWidget);
    });

    testWidgets('fires the primary action', (tester) async {
      var tapped = false;
      await tester.pumpWidget(
        host(
          LdNextActionCard(
      ...
      await tester.tap(find.text('Log protein'));
      expect(tapped, isTrue);
    });

    testWidgets('hides "Why?" when there is no evidence to show',
        (tester) async {
      const bare = NextAction(
        id: 'x',
        domain: ActionDomain.training,
        title: 'Workout starts in 45 minutes',
        subtitle: 'PUSH — 22 working sets',
        actionLabel: 'Start workout',
        deeplink: '/train',
        priority: 90,
      );
      await tester.pumpWidget(
        host(LdNextActionCard(action: bare, onAct: () {}, onWhy: () {})),
      );
