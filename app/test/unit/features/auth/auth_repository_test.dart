...
void main() {
  ...
  group('credential validation', () {
    ...
    test('sign-up requires a name, a valid email and 10+ characters', () async {
      ...
      expect(
        ...
        isA<ValidationFailure>().having((f) => f.code, 'code', 'name_required'),
      );
      expect(
        await attempt('not-an-email', 'longenough1', 'Sam'),
        isA<ValidationFailure>().having((f) => f.code, 'code', 'email_invalid'),
      );
      expect(
  ...
  group('local mode', () {
    ...
    test(
      'signing in twice keeps the same uid, so data is not orphaned',
      () async {
    ...
    test(
      'Google sign-in is unavailable rather than silently failing',
      () async {
        expect(
  ...
  group('cloud mode', () {
    ...
    test(
      'a successful sign-in maps the Firebase user onto a session',
      () async {
  ...
  group('ProfileRepository', () {
    ...
    test('the queued entry is routed to the user document', () async {
      ...
      await ProfileRepository(store: env.store, outbox: outbox).save(profile());

      expect(outbox.pending().single.collection, Outbox.profileCollection);
    });
    ...
    test(
      'pull without a backend reports it rather than returning empty',
      () async {
    ...
    test(
      'pull applies a newer remote profile and ignores a stale one',
      () async {
